...
logic on the R sides of the API without requiring access to the remote service.
...
remote iRODS server. The iRODS server should include the iRODS C++ REST API of 
0.9.3 or later - https://github.com/irods/irods_client_rest_cpp.
...
For miscellaneous development tasks one can, furthermore, use the none-exported 
...
`.Renviron` file. Enter your key b replacing `<key>` as follows:
...
Note, that the unit tests are build against a clean server with only one user 
