...
These Docker containers are designed to easily stand up a **DEMONSTRATION** of the iRODS server. It is intended for education and exploration.
...
In this example Alice is a user of iRODS and she can authenticate herself with `iauth()`. This prompts a dialog where you can enter your username and password without hardcoding this information in your scripts.
...
```{r filetypes, eval=requireNamespace("readr")}
library(readr)
...
```{r rmfile, include=FALSE, eval=requireNamespace("readr")}
unlink("foo.csv")
```

Later on somebody else might want to download this file again and store it locally:

```{r csv, eval=requireNamespace("readr")}
...
```{r clean, eval=requireNamespace("readr")}
# delete object
