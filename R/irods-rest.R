...
handle_irods_errors <- function(req) {
  httr2::req_retry(req, max_tries = 3, is_transient = ~ httr2::resp_status(.x) %in% c(429, 502, 503)) |>   # 502 nginx bad gateway
...
irods_errors <- function(resp) {
  ...
  if (!httr2::resp_status(resp) %in% c(429, 502, 503)) {
    ...
    paste0(httr2::resp_body_html(resp, check_type = TRUE)$error_message, " The iRODS server might be malconfigured.")
  } else {
