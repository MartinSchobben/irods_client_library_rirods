with_mock_dir("irods-rest-call", {
  test_that("irods rest call works", {
    ...
    # flags to curl call
    args <- list(
      `logical-path` = paste0(Sys.getenv("DEV_ZONE_PATH_IRODS") , "/", Sys.getenv("DEV_USER"), "/testthat"),
      stat = 1,
      permissions = 1,
      metadata = 1,
      offset = 0,
...
with_mock_dir("object-helpers", {
  test_that("irods object helpers work", {
    ...
    expect_false(is_collection(paste0(
      Sys.getenv("DEV_ZONE_PATH_IRODS"), "/", Sys.getenv("DEV_USER"), "/testthat/test"
    ))) # is a data object
    ...
    expect_error(is_object(paste0(
      Sys.getenv("DEV_ZONE_PATH_IRODS"), "/projectx/test"
    ))) # does not exist at all
  })
},
simplify = FALSE
