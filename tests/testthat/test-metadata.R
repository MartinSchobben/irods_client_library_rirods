with_mock_dir("metadata-1", {
  test_that("metadata works 1" , {
    ...
    # reference dataframe
    ref <- structure(list(
      logical_path = paste0(Sys.getenv("DEV_ZONE_PATH_IRODS"), "/", Sys.getenv("DEV_USER"), "/testthat/test"),
...
with_mock_dir("metadata-query", {
  test_that("metadata query works" , {
    ...
    # reference dataframe
    ref <- structure(list(
      collection = paste0(Sys.getenv("DEV_ZONE_PATH_IRODS"), "/", Sys.getenv("DEV_USER"), "/testthat"),
      data_object = "test"
    ),
    row.names = c(1L),
    class = "data.frame"
    ...
    expect_equal(
      iquery(
        paste0("SELECT COLL_NAME, DATA_NAME WHERE COLL_NAME LIKE '", Sys.getenv("DEV_ZONE_PATH_IRODS"), "/%'")
      ),
      ref
    )
